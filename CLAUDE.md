...
- **Same bump also needs the `?v=` cache-busting query param updated** on every `app.js`/`styles.css`/`hub.css`/`site-nav.js` reference across `index.html`, `news.html`, `support.html`, `privacy.html`, `cloud.html`, `delete-account.html` (added in v6.7.0, after a JS change didn't take effect for a signed-in admin testing right after a push — GitHub Pages/browser caching with no cache-buster meant they were still running the old cached app.js). Grep for the old version number and replace every hit in one pass rather than hand-editing each file.
...
- **Wedged client-side storage, not fixable by a normal cache clear**: one user hit persistently broken rendering (Latest Submissions empty, Featured cropped) in their main browser only, surviving hard refresh AND manually deleting the `mvg-cache` IndexedDB via DevTools on a normal tab. Root cause never fully confirmed, but it resolved only after installing the site as a PWA (`manifest.json` makes it installable) on Edge, then uninstalling that PWA and deleting its data — a more thorough wipe than a manual per-tab clear reaches. Likely explanation: an installed PWA's storage can live in a separate partition some browsers don't fully expose to DevTools on a regular (non-PWA) tab for the same origin. If this recurs: try "install as app" → uninstall it (with its data) as the nuclear option, before assuming it's a deploy/CDN problem.
...
## Voting system (current state, as of v5.60.0)

- **Voting itself is free and live**: any signed-in visitor can vote for any catalog video, repeatably (no per-person cap). `castVote()`/`voteForRowNum()` in `app.js` write immutable `voteEvents/{id}` docs; `functions/index.js`'s `onVoteEventCreated` denormalizes those into `videoVotes/{rowNum}` (public count/topVoter/latestVoter) and `voterTallies/{rowNum}_{uid}` (private, Admin-SDK-only). See `VOTING_FEATURE_EXPLAINER.md` (local-only, not committed) for a plain-language writeup.
- **Vote-credit wallet (prepaid, pay-per-vote) is scaffolded but not turned on** — `voteCredits` field, `createWalletCheckout`/`lemonSqueezyWebhook`/`castVoteWithCredit` Functions all exist and work, but `castVote()` is untouched (still free). Payment provider is **Lemon Squeezy**, not Stripe — Stripe doesn't support Philippines-registered merchant accounts, so that path was built, then fully ripped out and replaced. `WALLET_BUNDLES` in `functions/index.js` still has placeholder variant IDs pending real Lemon Squeezy products.
- **Admin vote management**: Vote Rounds view (`goAdminVoteRounds()`) can reset any video's votes to 0 (`resetVideoVotes` Function, with a confirm warning) — destructive, clears tallies too, unlike retirement below.
- **Vote retirement / Hall of Fame is built but fully dormant** (TRL-inspired — a video that camps in the top 5 for 14 cumulative days gets permanently retired and enshrined, never destructive). `checkVoteRetirements`/`unretireVideo` Functions exist, admin-triggered only (not scheduled), and nothing on the live site filters on the fields they write yet. Full design + activation checklist in `VOTE_RETIREMENT_PLAN.md` (local-only, not committed) — read that before touching anything vote-retirement-related.
- **TV Mode's Mirror/Interlace debug toggles were removed entirely** (not just hidden) — if you see `tv-mirror-btn`/`tv-interlace-btn`/`state.tv.mirror`/`state.tv.interlaceHz` referenced anywhere outside `CHANGELOG.md`, that's stale and should be deleted. TV Mode kept/gained: crop toggle, a widen/shrink toggle (mirrors the video-detail lightbox's own, same `.lightbox-panel.size-large` CSS), and a Vote button. The lightbox's own admin-only Mirror/Interlace debug tools (`.lightbox-mirror-btn`/`.lightbox-interlace-btn`) are a separate, still-live feature — don't remove those too.
- **Default playlists**: `seedDefaultPlaylists()` in `app.js` one-time-seeds a starter playlist per well-known director (see the constant list there) for every browser/account, guarded by a `mvg-default-playlists-seeded` localStorage flag. Ordinary playlists after creation — freely renamable/deletable, never re-seeded once removed.
